...
public class Office {
    ...
    public List<MeetingRoom> getEverySecondMeetingRoom(int number) {
        List<MeetingRoom> result = new ArrayList<>();  //result belső változó, nem érdekes h máshol is ez a neve
        if (number == 1) {
    ...
    public MeetingRoom getMeetingRoomWithGivenName(String name) {
        for (MeetingRoom meetingRoom : meetingRooms) {  //meghatározzuk a változót ami a lista tarta lmának tipusával egynelő : majd a lista végéig megyünk
    ...
    public List<MeetingRoom> getMeetingRoomsWithGivenNamePart (String namePart) {
        ...
        for (MeetingRoom meetingRoom: meetingRooms) {
            if (meetingRoom.getName().indexOf(namePart) >= 0){ //indexOf() -1 ha nincs benne, egyébként meg ahány karakter egyezik
